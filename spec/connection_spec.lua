describe("connection", function()
  ...
  it("connstring", function()
    ...
    assert.equal('t/storage/mongo', cache.typename[mongo])
    assert.equal('t/storage/mongo', cache.typename[mongo2])
    assert.equal('t/storage/mongo', cache.typename[mt(mongo2)])
    assert.equal('t/storage/mongo', cache.typename[cache.instance[mongo3]])
  ...
  it("connstring", function()
    assert.equal('mongodb://mongodb:27017/test', tostring(mongo.connection))
  ...
  it("connstring +MONGO_HOST", function()
    ...
    assert.equal('mongodb://othermongo:27017/test', tostring(mongo.connection))
    assert.equal(tostring(mongo.connection), tostring(mongo.connection()))
  end)
  it("connstring +MONGO_USER", function()
    t.env.MONGO_USER='user'
    assert.equal('mongodb://user@othermongo:27017/test', tostring(mongo.connection))
    assert.equal(tostring(mongo.connection), tostring(mongo.connection()))
  end)
  it("connstring +MONGO_PASS", function()
    t.env.MONGO_PASS='pass'
    assert.equal('mongodb://user:pass@othermongo:27017/test', tostring(mongo.connection))
    assert.equal(tostring(mongo.connection), tostring(mongo.connection()))
  end)
