...
return setmetatable({}, {
  __call=function(self, coll, query)
    ...
    if query=='*' or query=='' or type(query)=='nil' then query={} end
  ...
  __sub=function(self, query)
    if query=='*' or query=='' or type(query)=='nil' then query={} end
    if rawequal(self, query) then query={} end
    if query then return self.__:remove(query) end
  end,
